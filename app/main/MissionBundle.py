...
def get_classified_R(user_id, mission_id, weather_category, temperature_min, temperature_max, R_user_id, R_mission_id,
                     R_weather, R_temperature, R_rating, R_data_num):
...
def back_up(check_point, real_g, applicable_missions, mission_info, weekly_cost, N):
    ...
    return get_g_plus_h(check_point, weather_condition, applicable_missions, real_g, mission_info, i + 1, weekly_cost,
                        accumulated_cost, current_sack, h, N)
...
def get_g_plus_h(check_point, weather_condition, applicable_missions, real_g, mission_info, i, weekly_cost,
    ...
    if current_sack.index.size < 6:  # cost 다 써서 채웠는데 7개가 안되면 이번꺼 건너뛰고 다음꺼
        return get_g_plus_h(check_point, weather_condition, applicable_missions, real_g, mission_info, i + 1,
                            weekly_cost, accumulated_cost, current_sack, h, N)
        ...
        if required_weather_num > 0:
            if (mission_id in current_sack.index) == False:
                ...
                g_plus_h = get_g_plus_h(check_point, virtual_weather_condition, applicable_missions, g, mission_info,
                                        virtual_idx, weekly_cost, virtual_accmulated_cost, virtual_current_sack, 0, N)
                # print("g+h", g_plus_h)
                # print("best", best)
...
def set_user_applicable_missions(target_user_id, classified_R_hat, user_info, mission_info, weekly_weather):
    ...
    user_info.loc[target_user_id] = [a, b, c, applicable_missions, e]
...
def update_user_applicable_missions(user_id, user_info, mission_info, mission_id, activity, today_idx, weekly_weather):
    ...
    if activity == "done":
        ...
        print("update_applicable_missions", user_info.loc[user_id]['applicable_missions'])
...
def get_daily_mission(user_id, user_info, today_weather):
    if type(user_info.loc[user_id]['weekly_missions']) == type(None):
        return user_info.loc[user_id]['applicable_missions'].sort_values(by='cost').iloc[0].loc['mission_id']
...
def calculate_R_hat():
    ...
    classified_R = get_classified_R(user_info.index, mission_info.index, weather_category, temperature_min,
...
def getDailyMission():
    print(1)
    print("getDailyMission")
    global user_info, mission_info, classified_R_hat

    print(2)
    weather_category = ['sunny', 'cloudy']
    weathers = ['sunny', 'sunny', 'sunny', 'cloudy', 'cloudy', 'cloudy', 'cloudy']

    print(3)
    weekly_weather = get_weekly_weather(weathers, weather_category)
    print(weekly_weather)
    target_user_id = 75 #request.form['userIndex']
    ...
    print(7)
    print("weekly_missions\n", user_info.loc[target_user_id]['weekly_missions'])
    print(8)
    print("어플", user_info.loc[target_user_id]['applicable_missions'])
    print(9)
...
def incrementMission():
    ...
    if 'dislike' in request.form.keys():
        ...
        sql = "INSERT INTO MissionEvaluation (evaluationIndex,user,mission,rating) VALUES(%s,%s,%s,1)"
        try:
