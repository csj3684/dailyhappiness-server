...
from app.main.Evaluation import getEvaluation
...
def get_representative_value(discrete_distribution, scale, k, sigma, log_for_R_hat, user_id, mission_id):
    ...
    for i in mode.index:
        ...
        for j in discrete_distribution.index:
            if availability_for_representative_by_N(scale, k, i, mode.loc[i]['n'], j, discrete_distribution.loc[j]['n'],
                                                    sigma, log) == False:
...
def get_sacks(current_sack, g, accumulated_cost, missions, i):
    ...
    if (virtual_index < missions.index.size) and (len(current_sack) + virtual_index - i != N):
        h += ((weekly_cost - virtual_accumulated_cost) / missions.iloc[virtual_index].required_cost) * missions.iloc[
            virtual_index].expected_R
    # print("g+h : ", g + h)
    if g + h < best:
...
def get_R_hat_by_KNN(R, k, users_idx, missions_idx, data_num, log_for_R_hat, users_id, missions_id):
    R_hat = pd.DataFrame(index=users_idx, columns=missions_idx)
    log_for_R_hat.loc['KNN']['value'] = pd.DataFrame(index=['scale', 'sigma', 'distribution', 'graph_info'],
                                                     columns=['value'])
    ...
    for i in range(data_num):
        R_hat.loc[users_id.iloc[i]][missions_id.iloc[i]] = R.loc[users_id.iloc[i]][missions_id.iloc[i]]['rating']
    ...
    for i in range(len(users_idx)):
        ...
        for j in range(len(unexperienced_mission)):
            ...
            R_hat.loc[target_user_id][target_mission_id] = get_representative_value(k_nearest, scale, k, sigma,
                                                                                    log.loc['graph_info']['value'],
...
def get_knapsack(R_hat, users_idx, log_for_knapsack):
    ...
    for i in range(len(users_idx)):
        ...
        for i in range(sacks.index.size):
            mission_batches.append({'mission_id': sacks.iloc[i].sack, 'expected_rating': sacks.iloc[i].g,
                                    'total_cost': sacks.iloc[i].cost})
...
def getKnapsack():
    ...
    #print(users_id)
    # 중복을 없앤 유저, 미션 배열
    users_idx = data.drop_duplicates('users_id',keep='first').loc[:, 'users_id'].tolist()
    #print(users_idx)
    missions_idx = data.drop_duplicates('missions_id',keep='first').loc[:, 'missions_id'].tolist()
    #print(users_idx)
    #print(missions_idx)
    # 데이터 프레임 만들기
